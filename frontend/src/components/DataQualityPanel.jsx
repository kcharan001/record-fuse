    ...
    <div className="p-6 rounded-2xl bg-white border border-slate-200 space-y-4 shadow-sm">
      <div className="flex items-center justify-between border-b border-slate-100 pb-3">
        <div className="flex items-center gap-2 font-bold text-slate-900 text-sm">
          <DatabaseCheck className="w-5 h-5 text-indigo-600" />
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs font-mono">
        ...
        <div className="p-4 rounded-xl bg-slate-50 border border-slate-200 space-y-2">
          <span className="text-slate-600 font-bold block border-b border-slate-200 pb-1 uppercase tracking-wider text-[10px]">
            Data Integrity Assurances
          </span>
          <div className="flex justify-between py-1">
            <span className="text-slate-500">Missing Event IDs:</span>
            <span className={missingCount === 0 ? 'text-emerald-700 font-bold' : 'text-rose-600 font-bold'}>
              {missingCount}
            </span>
          </div>
          <div className="flex justify-between py-1">
            <span className="text-slate-500">Duplicate Event IDs:</span>
            <span className={duplicateCount === 0 ? 'text-emerald-700 font-bold' : 'text-rose-600 font-bold'}>
              {duplicateCount}
            </span>
          </div>
          <div className="flex justify-between py-1">
            <span className="text-slate-500">Invalid Provenance:</span>
            <span className={provErrors === 0 ? 'text-emerald-700 font-bold' : 'text-rose-600 font-bold'}>
              {provErrors}
            </span>
          </div>
        </div>
        ...
        <div className="p-4 rounded-xl bg-slate-50 border border-slate-200 space-y-2">
          <span className="text-slate-600 font-bold block border-b border-slate-200 pb-1 uppercase tracking-wider text-[10px] flex items-center gap-1">
            <Layers className="w-3.5 h-3.5 text-indigo-600" /> Clinical Concurrency Summary
          </span>
          <div className="flex justify-between py-1">
